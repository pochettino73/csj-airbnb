...
function drawHuecos() {{
  ...
  // Cabecera
  let h = '<div style="display:grid;grid-template-columns:32px 1fr 52px 70px 52px 52px 96px;gap:0;padding:6px 16px;margin-bottom:4px;">';
  ['#','Fechas','Noches','Temporada','Ant.','Occ.','RMS'].forEach(function(c,i) {{
    const al = i===0?'center':i>=4?'center':i===6?'right':'left';
  ...
  PRICING_GAPS.forEach(function(g, i) {{
    ...
    const occ   = g.future_occ_pct != null ? Math.round(g.future_occ_pct)+'%' : '—';
    const occCol= g.future_occ_pct>=70?'#22c55e':g.future_occ_pct>=40?'#f59e0b':'#ef4444';
    const dates = fmtD(g.start_date)+' &rarr; '+fmtD(g.end_date);
    h += '<div style="display:grid;grid-template-columns:32px 1fr 52px 70px 52px 52px 96px;gap:0;align-items:center;'
    ...
    // Ocupación futura
    h += '<div style="text-align:center;font-size:13px;font-weight:600;color:'+occCol+';">'+occ+'</div>';
    // RMS
