...
<div class="row r1">
  <div class="cd">
    ...
    <div class="s">Rating medio y n&ordm; reviews por periodo de evaluaci&oacute;n (365d). Verde = Superhost (&ge;4.8 + &ge;3 reviews)</div>
...
function drawC21() {{
  ...
  const barColors = isSH.map(v => v ? '#22c55ecc' : '#ef4444cc');
  charts.c21 = new Chart(document.getElementById('c21'), {{
    type:'bar',
    data: {{
      labels,
      datasets: [
        {{ label:'Reviews', data:counts, backgroundColor:barColors, borderRadius:4, borderSkipped:false, yAxisID:'y' }},
        {{ label:'Rating medio', data:ratings, type:'line', borderColor:'#f59e0b', borderWidth:2.5, pointRadius:4, pointBackgroundColor:ratings.map((v,i)=>isSH[i]?'#22c55e':'#ef4444'), tension:0.3, fill:false, yAxisID:'y1' }},
        {{ label:'Umbral 4.8', data:Array(labels.length).fill(4.8), type:'line', borderColor:'rgba(148,163,184,0.4)', borderDash:[6,4], borderWidth:1, pointRadius:0, fill:false, yAxisID:'y1' }},
      ]
    }},
    options: {{
      responsive:true, maintainAspectRatio:false,
      interaction:{{mode:'index',intersect:false}},
      plugins: {{
        legend:{{position:'top',labels:{{usePointStyle:true,font:{{size:10}},filter:i=>!i.text.includes('Umbral')}}}},
        tooltip:{{callbacks:{{
          afterBody: function(items) {{
            const i = items[0].dataIndex;
            return isSH[i] ? '\\n\\u2705 Superhost' : '\\n\\u274c No Superhost';
      ...
      scales: {{
        y: {{ position:'left', title:{{display:true,text:'Reviews',color:'#60a5fa'}}, ticks:{{color:'#60a5fa',stepSize:5}}, grid:{{color:GC}} }},
        y1: {{ position:'right', title:{{display:true,text:'Rating',color:'#f59e0b'}}, ticks:{{color:'#f59e0b'}}, grid:{{drawOnChartArea:false}}, min:4, max:5 }},
