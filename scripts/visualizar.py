...
function drawC5() {{
  ...
  const labels = M.slice(0,period);
  // Banda histórica con media en gris para distinguir del ADR amarillo de y2
  const bandGray = [
    {{ label:'Rango hist.', data:HMAX_PM.slice(0,period), borderColor:'transparent', backgroundColor:'rgba(148,163,184,0.07)', fill:'+1', pointRadius:0, pointHitRadius:0, order:10 }},
    {{ label:'_min', data:HMIN_PM.slice(0,period), borderColor:'transparent', backgroundColor:'transparent', fill:false, pointRadius:0, pointHitRadius:0, order:10 }},
    {{ label:'Media hist.', data:HAVG_PM.slice(0,period), borderColor:'rgba(148,163,184,0.6)', borderDash:[6,3], borderWidth:1.5, pointRadius:0, pointHitRadius:0, fill:false, order:9 }},
  ];
